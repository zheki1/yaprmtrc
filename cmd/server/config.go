// Твое решение, в принципе, жизнеспособно. 
// На реальных проектах для получения конфигурации из разных 
// источников обычно используется один из следующих подходов:

// использование пакета https://github.com/spf13/viper: 
// с его помощью можно удобно 
// считывать конфигуарцию из флагов, переменных окружения и файла одновременно
// создание трех объектов конфигурации -- из флагов, из переменных окружения 
// и из файла -- и их слияние при помощи например, пакета, 
// https://github.com/darccio/mergo
// Первый способ встречается чаще. 
// Кроме того на реальных пректах зачастую кофнигурации получается 
// только одним каким-то способом (через переменные оркужения, например), 
// но это не касается учебного проекта.
